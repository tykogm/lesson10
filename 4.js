...
// Решение
const compose = (...rest) => initialArgument => {

    return rest.reduceRight((accumulator, next, index) => {

        if (typeof next !== 'function') {
            throw new Error(`Argument ${i} isn't a function`);
        }
        if (accumulator === undefined && initialArgument !== undefined) {
            throw new Error(`callback at index ${index - 1} did not return any value.`);
        }
